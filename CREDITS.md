# 第三方资产致谢 / Third-Party Asset Credits

本项目使用了以下开源/CC 授权的素材。所有资产保留原作者署名，并遵循对应许可证条款。

---

## 3D 模型

### Gallery Room（自生成）
- **作者**: 项目自有
- **生成方式**: `scripts/generate_gallery.py`
- **格式**: glTF 2.0 单文件，buffer 内嵌为 base64
- **尺寸**: 40m × 8m × 25m 白色 PBR 盒子展厅
- **使用位置**: `entry/src/main/resources/rawfile/gltf/gallery/`
- **许可证**: 项目自有，无第三方依赖

### Damaged Helmet
- **作者 / Authors**: theblueturtle_ (original), Khronos Group (glTF sample)
- **来源 / Source**: https://github.com/KhronosGroup/glTF-Sample-Models/tree/master/2.0/DamagedHelmet
- **许可证 / License**: CC-BY 4.0
- **使用位置**: `entry/src/main/resources/rawfile/gltf/DamagedHelmet/`
- **用途**: 仅作 Phase 0 引擎能力验证，正式版本可移除。

---

## 引擎与框架

- **HarmonyOS / ArkGraphics 3D / ArkUI**: Huawei Device Co., Ltd. — Apache 2.0 部分组件
